        ...
        <div class="dj-panel__content">
            <v-list class="menu--mobile">
                <template v-for="menuItem in menuCategories">
                    <v-list-group v-if="menuItem.mega" :key="menuItem.url" no-action>
                        ...
                        <v-list>
                            <template v-for="megaItem in menuItem.megaContent[0].megaItems">
                                ...
                                >
                                    ...
                                    <MobileSubmenu :marginForChild="20" :menu="megaItem.megaContent[0].megaItems" />
                                </v-list-group>

                                <v-list-item-content class="subTextA childNavigation" :key="megaItem.url" v-else>
                                    <nuxt-link :to="megaItem.url" @click="handleClosePanel">
                                        {{ megaItem.text }}
                                    </nuxt-link>
                                </v-list-item-content>
