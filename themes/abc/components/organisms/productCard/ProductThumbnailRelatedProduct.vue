        ...
        <div class="p-2 d-flex flex-column gap-2 h-full">
            ...
            <nuxt-link :to="localePath(`/product/${productSku}`)">
                <transition mode="out-in">
                    <DjImage v-if="mainImage" :width="310" :height="310" :src="mainImage" :alt="productTitle" />
                </transition>
            </nuxt-link>
...
export default {
    ...
    computed: {
        ...
        offerExternalId() {
            return this.product.bestOffer.offerPrice.externalId;
        },
        ...
        mainImage() {
            const productsPics = this.$core.getProduct.getProductPictures(
                this.product.bestOffer.offerInventory.variant,
            );
            return productsPics.length > 1 ? productsPics.find((media) => media.isMain) : productsPics[0];
        },
        shippingTime() {
            if (this.product.bestOffer) {
        ...
        priceVisibilityStatus() {
            return (
                this.product.product.attributeValues.find((el) => el.externalId === 'Visibilité_Prix')?.value === 'Oui'
            );
        },
    },
    methods: {
