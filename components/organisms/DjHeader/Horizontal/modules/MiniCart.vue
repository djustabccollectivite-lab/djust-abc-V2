<template lang="html">
    <div class="dj-cart--mini">
        <nuxt-link class="header__extra" :to="localePath('/checkout/shopping-cart')">
            <i class="icon-cart"></i>
            <span>
                <i>{{ totalNumberOfProductsInCart }}</i>
            </span>
        </nuxt-link>
        <div v-if="totalNumberOfProductsInCart > 0" class="dj-cart__content">
...
export default {
    name: 'MiniCart',
    components: { Loading, DjMiniProductCart, DjLink },
    computed: {
        ...mapState({
        ...
        totalNumberOfProductsInCart() {
            ...
            //return this.$store.getters["cart/getTotalNumberCartItems"]
        },
    },
};
</script>
